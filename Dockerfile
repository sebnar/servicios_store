# Dockerfile simplificado para Frontend + Backend
FROM node:18-alpine
...
# Crear directorios
RUN mkdir -p /app/uploads /app/frontend/dist /var/log/nginx /etc/nginx/conf.d
...
CMD ["dumb-init", "sh", "-c", "cd /app/backend && npm start & sleep 5 && nginx -g 'daemon off;'"]
